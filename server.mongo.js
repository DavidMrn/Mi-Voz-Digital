...
app.get('/api/reportes/:id', async (req, res) => {
  try {
    ...
    const reporte = await Reporte.findOne({ $or: [{ id: q }, { _id: q }] }).lean();
...
app.post('/api/reportes/:id/votar', async (req, res) => {
  try {
    const q = req.params.id;
    const reporte = await Reporte.findOne({ $or: [{ id: q }, { _id: q }] });
...
app.get('/api/propuestas/:id', async (req, res) => {
  try {
    const q = req.params.id;
    const propuesta = await Propuesta.findOne({ $or: [{ id: q }, { _id: q }] }).lean();
...
app.post('/api/propuestas/:id/votar', async (req, res) => {
  try {
    const q = req.params.id;
    const propuesta = await Propuesta.findOne({ $or: [{ id: q }, { _id: q }] });
